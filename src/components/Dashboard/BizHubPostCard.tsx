...
const BizHubPostCard: React.FC<BizHubPostCardProps> = ({
  ...
  const handleClick = () => {
    router.push(`/feeds/biz-hub/${encodeURIComponent(name)}`); // Replace name with post id if available
  };
  return (
    <div
      className="bg-white rounded-xl shadow-sm p-6 flex flex-col gap-4 cursor-pointer hover:shadow-md transition"
      onClick={handleClick}
    >
