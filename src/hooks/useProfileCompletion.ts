...
import type { User, FullProfile } from "@/store/api";
...
export function useProfileCompletion(
  ...
  return useMemo(() => {
    ...
    // Professional
    if (profile?.professionalDetails?.companyName)
      categories["Professional"].completed++;
    if (profile?.professionalDetails?.industry)
      categories["Professional"].completed++;
    ...
    if (profile?.myBio?.mySkills && profile.myBio.mySkills.length > 0)
      categories["Bio"].completed++;
    if (profile?.myBio?.myAsk && profile.myBio.myAsk.length > 0)
      categories["Bio"].completed++;

    // Travel
    if (
      profile?.travelDiary?.myFootprints &&
      profile.travelDiary.myFootprints.length > 0
    )
      categories["Travel"].completed++;
    if (
      profile?.travelDiary?.businessBucketList &&
      profile.travelDiary.businessBucketList.length > 0
    )
      categories["Travel"].completed++;
    if (
      profile?.travelDiary?.dealsOnWheels &&
      profile.travelDiary.dealsOnWheels.length > 0
    )
      categories["Travel"].completed++;
    ...
    const totalFields = Object.values(categories).reduce(
